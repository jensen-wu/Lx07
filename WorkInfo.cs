  ...
  public  class WorkInfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activityName form workinfo join activity on workinfo.activity=activity.activityId where workId=@id";
        ...
        public IEnumerable<Model.WorkInfo>GetRecommend()
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过' and recomment='是' order by recommendTime desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public int GetFindCount(string workName)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select count(1) from workinfo where workverify='审核通过' and workName like count('%',@workName,'%')";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," +
